...
function getQuenStrength(nodeCTEntry)
	local nQuenLevel = 0;
	for _,nodeEffect in pairs(DB.getChildren(nodeCTEntry, "effects")) do
		...
		local name="";
		local val="";
		for token in string.gmatch(sEffectLabel, "[^%s]+") do
			if(name=="") then
				name = token;
			else
				if(val=="") then
		...
		if name == "QUEN" then
			Debug.chat("The Target is affected by Quen with a remaining value of " .. val);
			nQuenLevel = tonumber(val);
		end
...
function onEffectTextDecode(sEffect, rEffect)
	...
	local sUnits = s:match("%[UNITS ([^ (doble cerrada de corchete) +)]");
	if sUnits then
		s = s:gsub("%[UNITS ([^ (doble cerrada de corchete) +)]", "");
		if sUnits == "MIN" then
